...
role = "primary"
#role = "slave"
iptab_output_file = "/etc/iptables/parsec_rule.sh"
srv_ip = "65.21.243.37"
#conn = psycopg2.connect(user="dobro_dbuser",password="~6331dobro",host="31.13.20.190",port="5454",database="dobro")
...
try:
   ...
   with open(iptab_output_file,"w") as f:
      f.write (static_rule)
      cursor = conn.cursor()
      ...
      for data in cursor.fetchall():
         ...
         if role=="slave":
            ...
            iptab_rule2 += "iptables -t nat -A POSTROUTING -d "+dst_ip+" -p "+proto+" --dport "+w_port+" -j SNAT --to-source "+srv_ip+"\t# "+name+"\n"
      print (iptab_rule1)
      print (iptab_rule2)
